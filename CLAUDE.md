...
This file is your standing instructions for building **Lexi.kids** — a Dutch/Belgian adaptive learning platform for children in groep 1 t/m 8 — from the design references in `design_handoff/`.
...
1. Read `design_handoff/README.md` end to end. It is the source of truth for scope, tokens, routes, data model, and acceptance criteria.
2. Open `design_handoff/design/Lexi.kids v3.html` in a browser to see the live design before touching any code.
3. Skim the v3 JSX in `design_handoff/design/src/v3/` for the component you're about to implement. Lift exact values (colors, spacing, copy, layout). Do not re-invent.
